...
func (s *controllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	if err != nil {
		// This error indicates that a volume expansion is already in progress.
		// This can happen if a previous expansion request timed out on the client side
		// but was still processed by the backend.
		// Returning codes.DeadlineExceeded signals the csi-resizer to retry the
		// operation, allowing the ongoing expansion to complete.
		if lustre.IsUpdateInProgressErr(err) {
			msg := fmt.Sprintf("Expansion for volume %q to new capacity %d GiB is already in progress", volumeID, instance.CapacityGib)
			klog.V(4).Info(msg)

			return nil, status.Error(codes.DeadlineExceeded, msg)
		}

		// For other errors, we let the lustre error handler convert it to a gRPC status
		// and retry transient errors.
		klog.Errorf("UpdateInstance failed for volume %q: %v", volumeID, err)
