...
func TestControllerExpandVolume(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			expectErr: status.Error(codes.DeadlineExceeded, "Expansion operation ongoing for volume test-project/us-central1-a/updating-instance"),
		},
	}
...
func TestGetRequestCapacity(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "thin instance size",
			capRange: &csi.CapacityRange{
				RequiredBytes: thinInstanceSizeBytes,
			},
			expectCap: thinInstanceSizeBytes,
		},
		{
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			cap, err := getRequestCapacity(test.capRange)
			if test.expectErr == nil && err != nil {
				t.Errorf("test %q failed:\ngot error %q,\nexpected error nil", test.name, err)
			}
			if test.expectErr != nil && err == nil {
			...
			if cap != test.expectCap {
				t.Errorf("test %q failed:\ngot cap %d,\nexpected cap %d", test.name, cap, test.expectCap)
			}
		})
	}
}
