...
func handle() error {
		...
		if *doMultiNICSetup {
			...
			defer func() {
				if err := multiNICSubnetDelete(project, *gceZone, *gceRegion); err != nil {
					klog.Errorf("Failed to remove subnet %v: %w", multinicSubnetName, err)
				}
			}()
	...
	if *bringupCluster {
		if err := clusterUpGKE(project, *gceZone, *gceRegion, testParams.imageType, *numNodes, *useManagedDriver, *enableLegacyLustrePort, *doMultiNICSetup); err != nil {
