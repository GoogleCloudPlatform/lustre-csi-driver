...
func clusterUpGKE(project, gceZone, gceRegion, imageType string, numNodes int, useManagedDriver, enableLegacyLustrePort, multiNICSetup bool) error {
	locationArg, locationVal, err := gkeLocationArgs(gceZone, gceRegion)
	if err != nil {
		return err
	}
	...
	// if specify multi nic, create seperate pool with additional multi nic subnet.
	if multiNICSetup {
		nodePoolCmd := []string{"container", "node-pools", "create", multiNICNodePoolName,
			"--cluster=" + *gkeTestClusterName, locationArg, locationVal, "--additional-node-network",
			"network=" + *clusterNewtwork + ",subnetwork=" + multinicSubnetName}
		if useManagedDriver {
			nodePoolCmd = append(nodePoolCmd, "--addons", "LustreCsiDriver")
			if enableLegacyLustrePort {
				nodePoolCmd = append(nodePoolCmd, "--enable-legacy-lustre-port")
			}
		}
		if !useManagedDriver {
